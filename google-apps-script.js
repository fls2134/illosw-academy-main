...
const COMPANY_NAME = "일로 SW 입시센터";
const COMPANY_NAME_EN = "illo sw academy";

const EMAIL_SUBJECT_INQUIRY = `[${COMPANY_NAME}] 상담 문의`;
const EMAIL_BODY_INQUIRY_PREFIX = "새로운 상담 문의가 접수되었습니다.\n\n";
const EMAIL_BODY_INQUIRY_INFO = "문의 정보:\n";
const EMAIL_BODY_INQUIRY_FOOTER = "\n\n스프레드시트 링크: ";

const EMAIL_SUBJECT_REGISTER = `[${COMPANY_NAME}]에 신규 상담 요청이 있습니다.`;
const EMAIL_BODY_REGISTER_PREFIX = "새로운 상담 요청이 등록되었습니다.\n\n";
const EMAIL_BODY_REGISTER_FOOTER = "\n\n등록 정보:\n";

const SUCCESS_MESSAGE_INQUIRY = "상담 문의가 성공적으로 전송되었습니다.";
const SUCCESS_MESSAGE_REGISTER = "등록이 완료되었습니다.";
...
function sendInquiry(data) {
  try {
    ...
    try {
      const subject = EMAIL_SUBJECT_INQUIRY;
      const body =
        EMAIL_BODY_INQUIRY_PREFIX +
        EMAIL_BODY_INQUIRY_INFO +
        "- 이름: " +
        name +
        "\n" +
        "- 전화번호: " +
    ...
    return {
      success: true,
      message: SUCCESS_MESSAGE_INQUIRY,
    };
  } catch (error) {
    Logger.log("sendInquiry error: " + error.toString());
    throw error;
  }
}
...
function registerStudent(data) {
  try {
    ...
    studentsSheet.appendRow([newStudentSerial, name, number, 0]);

    currentSheet.appendRow([newStudentSerial, class_serial, timetable_serial]);

    try {
      const subject = EMAIL_SUBJECT_REGISTER;
      const body =
        EMAIL_BODY_REGISTER_PREFIX +
        "스프레드시트 링크: " +
        SPREADSHEET_URL +
        EMAIL_BODY_REGISTER_FOOTER +
        "- 이름: " +
        name +
        "\n" +
        "- 전화번호: " +
...
function doPost(e) {
  ...
  try {
    ...
    if (action === "register") {
      ...
      const result = sendInquiry(data);
      Logger.log("Inquiry result: " + JSON.stringify(result));
      return createCorsResponse(result);
    } else {
